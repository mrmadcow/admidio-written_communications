...
// Define Recipient addresses
    if ($getRoleSelect > 0 && $getRecipientMode == 'Role') {
        ...
        if (!$gCurrentUser->hasRightViewRole($getRoleSelect)) {
            throw new Exception('SYS_ROLE_SELECT_RIGHT', array($getRoleSelect));
        }
        ...
        if (count($arrMembers) > 0) {
            ...
            throw new Exception('Diese Rolle hat keine Mitglieder zugeordnet');
        }
    } else {
    ...
    // cancel current text run in main template adding required tags and nesting validated CKEditor string
